  ...
  return (
    <div className="sm:py-8 sm:px-8 px-4 py-4">
      ...
      <div className="mt-2">
        ...
        <div className="flex min-[500px]:flex-row gap-4 mb-6 flex-col">
          ...
          <div ref={filterRef} className="relative">
            <Button
              ...
              variant="outline"
              className="gap-2 bg-white p-5 text-[#2D2D2D]"
            >
              Filter by
          ...
          {hasActiveFilter && (
            <Button variant={"ghost"} onClick={clearFilters} className="flex gap-2 py-5 hover:bg-gray-100">
        ...
        <div className="mb-6">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {safeReportUi.length > 0 ? (
                safeReportUi.map((story) => <StoryCard key={story.id} story={story} />)
              ) : (
        ...
        {/* Pagination */}
        <div className={`flex items-center justify-between ${safeReportUi.length > 0 ? "" : "hidden"}`}>
          <Button variant="ghost" onClick={goToPreviousPage} disabled={!hasPrevious || isLoading}>
            ← Previous
          </Button>
          ...
          <Button variant="ghost" onClick={goToNextPage} disabled={!hasNext || isLoading}>
            Next →
          </Button>
        </div>
      </div>
